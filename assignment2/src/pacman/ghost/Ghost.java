...
public abstract class Ghost extends Entity {
    ...
    private void changeDirection(PacmanGame game) {
        Map<Direction, Double> distances = new TreeMap<>();
        for (Direction dir : Direction.values()) {
            //check the item stored on the next position is pathable.
            var item = game.getBoard().getEntry(getPosition().add(dir.offset()));
            if (item.getPathable()) {
                var nextPosition = game.getHunter().getPosition().add(dir.offset());
                distances.put(dir,Math.sqrt(Math.pow(nextPosition.getX() - getPosition().getX(),2) +
                                Math.pow(nextPosition.getY() - getPosition().getY(),2)));
            }
        }
        Direction[] Order = { Direction.UP, Direction.LEFT, Direction.DOWN, Direction.RIGHT };
        var minDistance = distances.values().iterator().next();
