...
public class PacmanBoard {
    ...
    public boolean equals(Object obj) {
        if (!(obj instanceof PacmanBoard)) {
            return false;
        }
        var other = (PacmanBoard)obj;
        if (this.getHeight() == other.getHeight() && this.getWidth() == other.getWidth()) {
            for (int i = 0; i < getHeight(); i++) {
                for (int j = 0; j < getWidth(); j++) {
                    if (this.getEntry(new Position(j,i)) == other.getEntry(new Position(j,i))) {
                        return true;
                    }
                }
            }
    ...
    @Override
    public int hashCode() {
        return this.width + this.height + Arrays.deepHashCode(this.board);
    }
    ...
    public String toString() {
        ...
        for (int i = 0; i < height - 1 ; i++) {
            for (int j = 0; j < width; j++) {
                result.append(getEntry(new Position(j,i)).getChar());
            }
            result.append(System.lineSeparator());
        }
        for (int i = 0; i < width; i++)
            result.append(getEntry(new Position(i,height-1)).getChar());
        return result.toString();
    }


    public static void main(String[] args) {
        PacmanBoard board = new PacmanBoard(4,4);
